...
const OGMemes = () => {
  ...
  const fetchOGMemes = async () => {
    try {
      ...
      if (!response.ok) {
        throw new Error("Failed to fetch OG memes")
      }
      const data = await response.json()
      setOgMemes(data)
    } catch (err) {
      setError("Failed to load OG memes. Make sure the Flask server is running!")
      console.error("Error fetching OG memes:", err)
    } finally {
      setIsLoading(false)
    }
  }
    ...
    return (
      <div className="flex items-center justify-center min-h-64">
        <div className="text-center">
          ...
          <p className="text-xl font-bold text-gray-600">Loading OG templates... 👑</p>
        </div>
      </div>
    )
  }
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-6xl mx-auto">
      ...
      {ogMemes.length === 0 ? (
        <div className="text-center p-8 bg-yellow-100 rounded-2xl">
          <p className="text-xl font-bold text-gray-600">No OG templates available yet! 🤷‍♂️</p>
        </div>
      ) : (
            ...
            >
              <div className="relative">
                <img
                  ...
                  alt={`OG meme template ${index + 1}`}
                  className="w-full h-64 object-cover transition-transform duration-200 group-hover:scale-105"
                />
                <div className="absolute top-2 left-2">
                  <span className="bg-yellow-300 text-purple-800 font-black px-2 py-1 rounded-lg text-sm">OG 👑</span>
                </div>
